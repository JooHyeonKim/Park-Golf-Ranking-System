...
function buildOverviewRows(tournament, genderFilter, holeCount, clubLabel) {
  ...
  if (hasScoreD) { header.push(is54Hole ? 'D1코스' : 'D코스'); header.push('CD총계'); }
  if (hasScoreEF) { header.push('A2코스'); header.push('B2코스'); header.push('EF총계'); }
  header.push('홀인원');
  ...
  for (const player of players) {
    ...
    if (!is27Hole) row.push((player.scoreA != null && player.scoreB != null) ? player.scoreA + player.scoreB : '');
...
function buildIndividualRows(tournament, clubLabel) {
  const ranked = calculateRankings(tournament.players);
  ...
  const rows = [[`남자_${clubLabel}`, '남자_성명', '남자_타수', '순위', `여자_${clubLabel}`, '여자_성명', '여자_타수']];
  ...
  if (holeInOnePlayers.length > 0) {
    ...
    holeInOnePlayers.forEach((player, index) => {
      rows.push([index + 1, player.club, player.name, player.gender, player.holeInOne === true ? '-' : player.holeInOne]);
    });
  }
  return rows;
}

function buildEncouragementRows(tournament, maleMaxRank, femaleMaxRank, clubLabel) {
  const ranked = calculateRankings(tournament.players);
  const withScores = ranked.filter(p => p.name && calculateTotal(p) !== null);
...
function buildTeamRows(tournament, clubLabel) {
  const excludeCount = 5;
  const ranked = calculateRankings(tournament.players);
  const withScores = ranked.filter(p => p.name && calculateTotal(p) !== null);
  const excludedIds = new Set([
    ...withScores.filter(p => p.gender === '남').slice(0, excludeCount).map(p => p.id),
    ...withScores.filter(p => p.gender === '여').slice(0, excludeCount).map(p => p.id),
  ]);
  ...
  const teamRankings = clubResults.map((club, index) => {
    if (club.total !== previousTotal) { currentRank = index + 1; previousTotal = club.total; }
    return { ...club, rank: currentRank };
  });

  const rows = [['순위', `${clubLabel}명`, '합계', '1성명', '1타수', '2성명', '2타수', '3성명', '3타수', '4성명', '4타수']];
...
export function useExcelDownload(tournament, maleMaxRank = 10, femaleMaxRank = 10) {
  ...
  const handleExcelDownload = useCallback(async () => {
    ...
    try {
      // 동적 import: 버튼 클릭 시에만 로드되어 PDF/html2canvas에 영향 없음
      ...
      const addSheet = (rows, name) => XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(rows), name);

      addSheet(buildOverviewRows(tournament, null, holeCount, clubLabel), '전체현황');
      addSheet(buildOverviewRows(tournament, '남', holeCount, clubLabel), '전체현황_남');
      addSheet(buildOverviewRows(tournament, '여', holeCount, clubLabel), '전체현황_여');
      addSheet(buildIndividualRows(tournament, clubLabel), '개인전');
      addSheet(buildEncouragementRows(tournament, maleMaxRank, femaleMaxRank, clubLabel), '장려상');
      addSheet(buildTeamRows(tournament, clubLabel), '단체전');
