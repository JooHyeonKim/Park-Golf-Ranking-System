...
단체전 탭은 클럽별 팀 집계 기능을 제공한다. 개인전 수상자를 제외한 선수들을 클럽별로 그룹화하여 상위 5명의 타수 합계로 순위를 매긴다. 수상자 제외 범위는 드롭다운 셀렉터로 선택 가능하다 (전체 포함 / 수상자 1위~7위까지 제외, 기본값: 5위까지 제외).
...
2. 이름+점수 있는 선수 필터링
3. 개인전 수상자 제외 대상 추출 (excludeCount 기반):
   - excludeCount=0: 전체 포함 (제외 없음)
   - excludeCount=1~7: 남녀 각 상위 N명 → excludedIds (Set)
...
- `excludeCount` state (기본값 5)로 제외 범위 관리
- `<select>` 드롭다운 UI: 전체 포함(0), 수상자 1위~7위까지 제외(1~7)
- 선택 변경 시 useMemo 의존성에 의해 테이블 즉시 재계산
