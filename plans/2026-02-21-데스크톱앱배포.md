...
## 배포 결과 (desktop-v1.0.0)

### GitHub Actions 빌드
| Job | 상태 | 소요 시간 |
|-----|------|-----------|
| build-mac | 성공 | 1m 43s |
| build-windows | 성공 | 1m 43s |
| create-release | 성공 | 23s |
| **전체** | **성공** | **2m 11s** |

### 빌드 산출물
| 플랫폼 | 파일 | 크기 |
|--------|------|------|
| macOS | `.dmg` (universal: x64 + arm64) | 198 MB |
| Windows | `.exe` (NSIS 인스톨러, x64) | 92.6 MB |

### 다운로드 URL
```
https://github.com/JooHyeonKim/Park-Golf-Ranking-System/releases/tag/desktop-v1.0.0
```

### 클라이언트 설치 안내

#### macOS (Gatekeeper 차단 해결)
코드 서명이 없어 macOS Gatekeeper가 앱을 차단함. 아래 중 하나로 해결:

**방법 1: 터미널 명령 (가장 확실)**
1. DMG 더블클릭 → 앱을 Applications 폴더로 드래그
2. 터미널 열기 (Launchpad > 기타 > 터미널)
3. 아래 명령어 붙여넣기 후 Enter:
```bash
xattr -cr /Applications/파크골프\ 점수판.app
```
4. Applications에서 앱 더블클릭 → 정상 실행

**방법 2: 시스템 설정**
1. 앱을 한 번 실행 시도 (차단됨)
2. 시스템 설정 > 개인정보 보호 및 보안
3. 하단 "파크골프 점수판이(가) 차단되었습니다" 확인
4. "확인 없이 열기" 클릭

#### Windows
- SmartScreen 경고 → "추가 정보" → "실행" 클릭
- 코드 서명 미설정 상태 (향후 Apple Developer / Windows EV 인증서로 해결 가능)
...
- [x] macOS DMG 패키징 로컬 테스트 통과
- [x] GitHub Actions Mac/Win 빌드 성공
- [x] GitHub Releases 배포 완료 (desktop-v1.0.0)
